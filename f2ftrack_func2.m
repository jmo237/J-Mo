function [x1,y1,maxValueHistory]=f2ftrack_func2(image_vol,x1,y1,psize,totalFrameNumber,totalPointNumber,maxValueHistory)

h = waitbar(0,'tracking points..');
npsize=round(psize*1.1);
for nf=1:totalFrameNumber-1
    ...
    for np=1:totalPointNumber
        ...
        maxValue=max(patch_c(:));
        [xrow, ycol] = find(ismember(patch_c, maxValue));
